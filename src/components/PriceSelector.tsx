...
export function PriceSelector({
  ...
  const priceOptions = useMemo(() => getPriceOptions(histogramData?.range), [histogramData]);

  const isOptionDisabled = (price: number | null, dropdownType: PriceType): boolean => {
  ...
  const PriceInput = ({ type }: { type: PriceType }) => {
    ...
    return (
      <div className={`flex-1 flex flex-col gap-2 ${!isModal ? "max-w-[200px]" : ""} relative`}>
        ...
        {activeDropdown && (
          ...
          >
            <AnimatePresence mode="wait">
              <motion.div
                key={activeDropdown}
                initial={{ x: activeDropdown === "max" ? SLIDE_DISTANCE : -SLIDE_DISTANCE }}
                animate={{ x: 0 }}
                exit={{ x: activeDropdown === "max" ? -SLIDE_DISTANCE : SLIDE_DISTANCE }}
                transition={{ duration: SLIDE_DURATION }}
                className="flex flex-col overflow-hidden"
              >
                {priceOptions.map((price) => renderOption(price, activeDropdown))}
              </motion.div>
            </AnimatePresence>
          </motion.div>
        )}
